...
def reprocesar_predicciones_route():
    ...
    print(f"[DEBUG] header recibido: {repr(secret_header)} (len={len(secret_header) if secret_header else 0})", flush=True)
    print(f"[DEBUG] secret esperado: {repr(jobs_secret)} (len={len(jobs_secret) if jobs_secret else 0})", flush=True)
    print(f"[DEBUG] coinciden: {secret_header == jobs_secret}", flush=True)
